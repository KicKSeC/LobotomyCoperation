...
namespace LobotomyCoperation.Rimworld
{
    public sealed class AbnormalyCodex : IExposable
    {
        private Dictionary<AbnormalyCategoryDef, bool> hiddenCategories;

        private Dictionary<AbnormalyCategoryDef, bool> hiddenEntries;

        private HashSet<ThingDef> discoveredAbnormalies;

        public bool debug_UnhideAllResearch;

        public const string AbnormalyDiscoveredSignal = "AbnormalyDiscovered";

        private const int AbnormalyDiscoveredLetterDelayTicks = 600;
        ...
        public bool Hidden(ResearchProjectDef def)
        {
            ...
            if (def.knowledgeCategory != null && Find.Anomaly.Level < 1)
            {
                return true;
            }
            foreach (EntityCodexEntryDef allDef in DefDatabase<EntityCodexEntryDef>.AllDefs)
        ...
        public void SetDiscovered(EntityCodexEntryDef entry, ThingDef discoveredDef = null, Thing discoveredThing = null)
        {
            ...
            if (Current.ProgramState != ProgramState.Playing || Find.Anomaly.Level <= 0)
            {
                return;
            }
