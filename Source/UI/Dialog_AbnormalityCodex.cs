...
{
    // define new Abnormality Codex window
    public class Dialog_AbnormalityCodex : Window
    {
        ...
        private AbnormalityCodexEntryDef selectedEntry;

        private List<AbnormalityCategoryDef> categoriesInOrder;

        private Dictionary<AbnormalityCategoryDef, List<AbnormalityCodexEntryDef>> entriesByCategory = new Dictionary<AbnormalityCategoryDef, List<AbnormalityCodexEntryDef>>();
        ...
        public Dialog_AbnormalityCodex(AbnormalityCodexEntryDef selectedEntry = null)
        {
            doCloseX = true;
            doCloseButton = true;
            forcePause = true;
            categoriesInOrder = DefDatabase<AbnormalityCategoryDef>.AllDefsListForReading.OrderBy((AbnormalityCategoryDef x) => x.listOrder).ToList();
            ...
            this.selectedEntry = selectedEntry ?? DefDatabase<AbnormalityCodexEntryDef>.AllDefs.OrderBy((AbnormalityCodexEntryDef x) => x.label).FirstOrDefault((AbnormalityCodexEntryDef x) => x.Discovered);
        }
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            {
                Widgets.Label(new Rect(0f, 0f, rect.width, 30f), "AbnormalityCodex".Translate());
            }
            ...
            rect.yMin += 40f;
            TaggedString taggedString = "AbnormalityCodexDesc".Translate();
        ...
        private void LeftRect(Rect inRect)
        {
            ...
            if (selectedEntry != null)
            {
                ...
                using (new TextBlock(GameFont.Medium))
                {
                    Widgets.Label(new Rect(0f, num, viewRect.width, 30f), flag ? selectedEntry.LabelCap : "UndiscoveredAbnormality".Translate());
                    num += 40f;
                }
                using (new TextBlock(newWordWrap: true))
                {
                    string text = (flag ? selectedEntry.Description : ((string)"UndiscoveredAbnormalityDesc".Translate()));
                ...
                if (flag)
                {
                    if (selectedEntry.linkedThings.Count > 0)
                    {
                        foreach (ThingDef linkedThing in selectedEntry.linkedThings)
                        {
                            ...
                            if (devShowAll || Find.AbnormalityCodex.Discovered(linkedThing))
                            {
                ...
                else if (Prefs.DevMode && DebugSettings.godMode)
                {
                    if (Widgets.ButtonText(new Rect(0f, num, viewRect.width, ButSize.y), "DEV: Discover"))
                    {
                        if (!selectedEntry.linkedThings.NullOrEmpty())
                        {
                            ...
                            {
                                Find.AbnormalityCodex.SetDiscovered(selectedEntry, selectedEntry.linkedThings[i]);
                            }
                        }
                        else
                        {
                            Find.AbnormalityCodex.SetDiscovered(selectedEntry);
                        }
                    }
                    num += ButSize.y;
                }
        ...
        private void DrawEntry(Rect rect, AbnormalityCodexEntryDef entry, bool discovered)
        {
